...
public class StoneLayerHandler : BlockLayerHandler
{
    ...
    public int Snow_Stone_Threshold = 50;

    [SerializeField]
    private NoiseSettings stoneNoiseSettings;

    public DomainWarping StoneWarping;
    ...
    protected override bool TryHandling(ChunkData chunkData, int x, int y, int z, int surfaceHeightNoise, Vector2Int mapSeedOffset)
    {
        ...
        float stoneNoise = StoneWarping.GenerateDomainNoise(chunkData.worldPosition.x + x, chunkData.worldPosition.z + z, stoneNoiseSettings);
        ...
        if (stoneNoise > stoneThreshold)
        {
            ...
            {
                if(i == endPosition && i >= Snow_Stone_Threshold)
                {
                    Chunk.SetBlock(chunkData, new Vector3Int(x, i, z), BlockType.Snow_Stone);
                    continue;
                }
